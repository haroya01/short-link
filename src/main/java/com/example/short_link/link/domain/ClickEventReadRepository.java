...
/**
 * Analytics/reporting projections off click_event. Split out of {@link ClickEventRepository} so the
 * write-side stays a CRUD-shaped Repository while the read-side advertises its real shape — 40+
 * aggregate queries returning projection records, used exclusively by *QueryService callers.
 *
 * <p>Plan D5 Q2 trigger: 2+ table JOIN / same data in many shapes / aggregate functions. Q3
...
public interface ClickEventReadRepository extends Repository<ClickEventEntity, Long> {
  ...
  // Campaign 용 multi-link 변형 — 한 캠페인의 모든 batch link 를 한 번에 aggregate. since 는 보통
  // campaign.startsAt 으로 호출되어 *캠페인 시작 이후* 클릭만 집계 (test scan 제외).
  @Query(
      "SELECT FUNCTION('HOUR', FUNCTION('CONVERT_TZ', c.clickedAt, '+00:00', :tz)) AS hour, "
          + "COUNT(c) AS count "
