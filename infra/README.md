...
- ECS Fargate (ARM64, 0.25 vCPU / 1 GB) × 1 task — public subnet, public IP 자동 할당
  - 컨테이너 2개: `app` (Spring Boot, 768 MB) + `redis` 사이드카 (alpine, 192 MB) — 같은 task 안에서 localhost:6379로 통신
...
월 비용: 약 $45 (ALB $18 + Fargate $11 + RDS $15 + 기타 $1).

> Redis는 task 내 사이드카라 task 재시작 시 휘발. rate-limit / Spring cache / refresh token 저장이 초기화되니 사용자는 재로그인 필요. desired_count=1 단일 task라 cross-instance 캐시 일관성 문제는 없음.
...
ECR이 비어 있으므로 ECS 태스크는 일단 풀에 실패한다 (이건 정상). VPC/RDS/ALB/ACM 등 모든 인프라는 생성된다.
...
`db_password`는 random_password로 자동 생성됨. 나머지는 placeholder `REPLACE`로 채워져 있으니 직접 업데이트:
...
다시 켤 때 `--desired-count 1`. RDS는 stop이 single-AZ에서는 7일 한도이므로 장기 정지하려면 destroy.
...
- Redis는 같은 task 내 사이드카 컨테이너 (`redis:7-alpine`). 영속성 없음, maxmemory 100MB allkeys-lru.
